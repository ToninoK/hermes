...
async def get_schedules(frequency, email):
    async with conn.cursor() as cur:
        await cur.execute("SELECT * FROM scheduled_reports WHERE frequency = %s AND created_by=%s", (frequency, email))
...
async def get_schedules_all(frequency):
    async with conn.cursor() as cur:
        await cur.execute("SELECT * FROM scheduled_reports WHERE frequency = %s", (frequency,))
        results = await cur.fetchall()
        return [dict(schedule) for schedule in results]
